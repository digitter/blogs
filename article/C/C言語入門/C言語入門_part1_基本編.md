...
- [算術演算](#算術演算)
  ...
  - [接頭と接尾のインクリメントとデクリメントの違い](#接頭と接尾のインクリメントとデクリメントの違い)
...
メモリのアドレスと取得するには アンパサンド `&` を利用します。

scanf が受け取った入力値をそのアドレスに記録してくれます。
...
しかし、注意が必要なのは、配列の場合は `&` は不要なケースがあります。
配列は先頭要素を示す様に暗黙に型変換されています。
...
要素の値を設定する場合、明示的にアドレスを指定する必要がある。
